...
class BBPasswordChangeview(SuccessMessageMixin, LoginRequiredMixin, PasswordChangeView):
    template_name = 'salary/layout/password_change.html'
    success_url = '/'
...
class GetFile(APIView):
    def post(self, request, format=None):
        ...
        for i in range(0, len(request.data['FIO'])):
            ...
            worksheet.write(f'E{str(i + 2)}', request.data['Sort'][i])
...
class ChangePassword(APIView):
    def post(self, request, format=None):
        for x in User.objects.exclude(pk=1):
            x.set_password(request.data['text_password'])
            x.save()
        return Response()
...
class ChangeData(APIView):
    def post(self, request, format=None):
        ...
        elif request.data['name_field'] == 'sort':
            get_dates[request.data['worker']][3][10][request.data['name_field']] = f'{request.data["value"]}'
...
def update_status_time_sheet(request, pk, status):
    ...
    if status == 'close' and pre_status != 'opened':
        ...
        next_year = int(timeSheet.dataSheet.year) if timeSheet.dataSheet.month < 12 else int(
            timeSheet.dataSheet.year) + 1
        TimeSheet.objects.create(
            dates=create_dates(Department.objects.get(foreman=request.user.pk).pk, next_month, next_year),
...
class LoadTimeSheetByTime(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['months'] = ['Январь', 'Февраль', 'Март', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август',
                                 'Сентябрь',
    ...
    def post(self, *args, **kwargs):
        ...
        if timesheet.status == 'open':
            timesheet.save()
        return redirect(
            '/salary/timesheet/{}/{}'.format(timesheet.department.pk, timesheet.dataSheet.strftime("%Y-%m-%d")))
...
class LoadTimeSheet(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not TimeSheet.objects.filter(department=Department.objects.get(foreman=self.request.user.pk)).exists():
            TimeSheet.objects.create(
                dates=create_dates(Department.objects.get(foreman=self.request.user.pk).pk, dt.date.today().month,
                                   dt.date.today().year),
        ...
        if len(TimeSheet.objects.filter(
                Q(department=user_depart.pk, status='open') | Q(department=user_depart.pk, status='opened'))) == 1:
...
class PayRoll(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        get_all_time_sheets = TimeSheet.objects.filter(
        ...
        if (not Coefficient.objects.filter(
        ...
        for time_sheet in get_all_time_sheets:
            if (not Payroll.objects.filter(department=time_sheet.department,
                                           time_sheet__dataSheet__month=dt.datetime.strptime(
                                               self.kwargs['request_date'], "%Y-%m-%d").month,
        ...
        if Group.objects.get(user=self.request.user.pk).name == 'Администрация' or Group.objects.get(
                user=self.request.user.pk).name == 'Бухгалтера':
    ...
    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(**kwargs)
        context['errors'] = Department.objects.filter(
            pk__in=[pay_dep.department.pk for pay_dep in TimeSheet.objects.filter(
                dataSheet__year=dt.datetime.strptime(self.kwargs['request_date'], "%Y-%m-%d").year,
                dataSheet__month=dt.datetime.strptime(self.kwargs['request_date'], "%Y-%m-%d").month,
                status='open',
        ...
        context['group'] = Group.objects.get(user=self.request.user.pk)
        try:
            context['coefficient_worker'] = Coefficient.objects.get(
                date_create__year=dt.datetime.strptime(self.kwargs['request_date'], "%Y-%m-%d").year,
    ...
    def post(self, *args, **kwargs):
        ...
        if self.request.POST['status'] == 'pass':
            background = '#ffff00b3'
            dates[str(worker)][3][11]['qwe'] = background
        if self.request.POST['status'] == 'absenteeism':
            background = '#ff0004b3'
            dates[str(worker)][3][11]['qwe'] = background
        if self.request.POST['status'] == 'weekend':
        ...
        timesheet.dates = json.dumps(dates)
        timesheet.save()
        return redirect('/salary/payroll/{}'.format(self.request.POST['date']))
...
class AddWorker(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.department = TimeSheet.objects.filter(
            foreman='{} {}'.format(self.request.user.last_name, self.request.user.first_name)).last().department
        form.save()
        return redirect('/salary/timesheet')
